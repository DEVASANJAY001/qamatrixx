...
const ScoreInput = ({ value, onChange, defectRating }: { value: number | null; onChange: (v: number | null) => void; defectRating: number }) => (
  <input
    ...
    className={`w-full text-center font-mono text-xs py-1 rounded border-0 focus:ring-1 focus:ring-primary outline-none ${
      value !== null && value >= defectRating ? "text-emerald-600 font-semibold bg-emerald-500/5" :
                ...
                >
                  ...
                  <td className="px-2 py-1.5 text-center" onClick={(e) => e.stopPropagation()}>
                    <select
                      ...
                      className={`w-8 h-6 text-center font-bold text-[10px] rounded-full border-0 cursor-pointer appearance-none focus:ring-1 focus:ring-primary outline-none ${
                        entry.defectRating === 5 ? "bg-destructive/15 text-destructive" :
                  ...
                  {entry.weeklyRecurrence.map((val, i) => (
                    <td key={`w${i}`} className="px-0.5 py-0.5 border-l border-border bg-amber-50/10" onClick={(e) => e.stopPropagation()}>
                      <input
                        ...
                        className={`w-full text-center font-mono text-xs py-1 rounded border-0 focus:ring-1 focus:ring-primary outline-none ${
                          val > 0 ? "bg-destructive/10 text-destructive font-bold" : "bg-transparent text-muted-foreground"
                        }`}
                      />
                    </td>
                  ))}
                  ...
                  {trimKeys.map(k => (
                    <td key={k} className="px-0.5 py-0.5 border-l border-border">
                      <ScoreInput
                        ...
                        onChange={(v) => onScoreUpdate?.(entry.sNo, "trim", k, v)}
                      />
                    </td>
                  ))}
                  {/* Editable Chassis */}
                  {chassisKeys.map(k => (
                    <td key={k} className="px-0.5 py-0.5 border-l border-border">
                      <ScoreInput
                        value={entry.chassis[k]}
                        defectRating={entry.defectRating}
                        onChange={(v) => onScoreUpdate?.(entry.sNo, "chassis", k, v)}
                      />
                    </td>
                  ))}
                  {/* Editable Final (F10-FPQG only) */}
                  {finalKeysDisplay.map(k => (
                    <td key={k} className="px-0.5 py-0.5 border-l border-border">
                      <ScoreInput
                        value={entry.final[k]}
                        defectRating={entry.defectRating}
                        onChange={(v) => onScoreUpdate?.(entry.sNo, "final", k, v)}
                      />
                    </td>
                  ))}
                  {/* Residual Torque (standalone) */}
                  <td className="px-0.5 py-0.5 border-l-2 border-rose-400">
                    <ScoreInput
                      value={entry.final.ResidualTorque}
                      defectRating={entry.defectRating}
                      onChange={(v) => onScoreUpdate?.(entry.sNo, "final", "ResidualTorque", v)}
                    />
                  </td>
                  {/* Q'Control 1.1-5.3 */}
                  {qControlLabels.map(q => (
                    <td key={q.key} className="px-0.5 py-0.5 border-l border-border">
                      <ScoreInput
                        value={entry.qControl[q.key]}
                        defectRating={entry.defectRating}
                        onChange={(v) => onScoreUpdate?.(entry.sNo, "qControl", q.key, v)}
                      />
                    </td>
                  ))}
                  {/* Q'Control Detail: CVT, SHOWER, Dynamic/UB, CC4 */}
                  {qControlDetailKeys.map(q => (
                    <td key={q.key} className="px-0.5 py-0.5 border-l border-border">
                      <ScoreInput
                        value={entry.qControlDetail[q.key]}
                        defectRating={entry.defectRating}
                        onChange={(v) => onScoreUpdate?.(entry.sNo, "qControlDetail", q.key, v)}
                      />
                    </td>
                  ))}
                  {/* Control Rating */}
                  ...
                  <td className="px-1 py-1.5 text-center border-l border-primary/30">
                    <StatusBadge status={entry.workstationStatus} />
                  </td>
                  <td className="px-1 py-1.5 text-center">
                    <StatusBadge status={entry.mfgStatus} />
                  </td>
                  <td className="px-1 py-1.5 text-center">
                    <StatusBadge status={entry.plantStatus} />
                  </td>
                  <td className="px-1 py-1.5 text-center" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-0.5 justify-center">
                      {editingRow === entry.sNo ? (
                        <button
                          onClick={() => {
                            Object.entries(editFields).forEach(([field, value]) => {
                              onFieldUpdate?.(entry.sNo, field, value);
                            });
                            setEditingRow(null);
                            setEditFields({});
                          }}
                        ...
                        <button
                          onClick={() => {
                            setEditingRow(entry.sNo);
                            setEditFields({
                              source: entry.source,
                              operationStation: entry.operationStation,
                              designation: entry.designation,
                              concern: entry.concern,
                              mfgAction: entry.mfgAction,
                              resp: entry.resp,
                              target: entry.target,
                            });
                          }}
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`Delete concern #${entry.sNo}?`)) {
                            onDeleteEntry?.(entry.sNo);
                          }
                        }}
                          ...
                          {editingRow === entry.sNo ? (
                            <>
                              ...
                              <div className="space-y-1">
                                ...
                                <input value={editFields.designation ?? ""} onChange={(e) => setEditFields(f => ({ ...f, designation: e.target.value }))} className="w-full px-2 py-1 text-xs border border-input rounded bg-background" />
                              </div>
                            </>
                          ) : (
                            <>
